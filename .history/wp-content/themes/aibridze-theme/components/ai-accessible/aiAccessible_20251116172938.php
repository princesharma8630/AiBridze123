<!-- AI Accessible Section -->
<section class="ai-accessible-section">
    <div class="ai-accessible-container">
        
        <!-- Main Heading -->
        <div class="ai-accessible-content">
            <h2 class="ai-accessible-heading">
                At AiBridze, we make cutting-edge AI accessible. <br>
                Interact with a <span class="gradient-text">live neural network demo</span> directly in <br>
                your browser — no setup, no risk. Just explore, <br>
                learn, and witness the power of AI firsthand.
            </h2>
        </div>

        <!-- Scrolling Ticker -->
        <div class="ticker-wrapper">
            <div class="ticker-content" id="tickerContent">
                <span class="ticker-text">Innovate with Artificial Intelligence. Automate. Accelerate. Achieve. Smarter Solutions. Real Results. Predict. Analyze. Transform. Your Business. AI-Powered. </span>
                <span class="ticker-text">Innovate with Artificial Intelligence. Automate. Accelerate. Achieve. Smarter Solutions. Real Results. Predict. Analyze. Transform. Your Business. AI-Powered. </span>
            </div>
        </div>

        <!-- Partner Logos -->
        <div class="ai-accessible-logos">
            <div class="logo-item">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/partners/metriks.png" alt="Metriks Data Center" />
            </div>
            <div class="logo-item">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/partners/quo-legal.png" alt="QUO Legal Firm" />
            </div>
            <div class="logo-item">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/partners/mnmize.png" alt="M'N'MIZE Interior" />
            </div>
            <div class="logo-item">
                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/partners/nd2.png" alt="ND2 Nordyne Defense Dynamics" />
            </div>
        </div>

    </div>
</section>