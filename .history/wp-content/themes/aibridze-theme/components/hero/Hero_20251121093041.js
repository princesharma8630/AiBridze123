...
jQuery(document).ready(function($) {
        ...
        Starfield.prototype = {
            ...
            anim: function(){
                ...
                for(var i = 0; i < this.n; i++) {
                    ...
                    // ============================================
                    // CHANGED: Draw DOTS instead of LINES
                    // ============================================
                    ...
                    && this.test) {
                        ...
                        var opacity = 1 - this.star_color_ratio * this.star[i][2];
                        
                        // Draw main dot/circle
                        this.context.beginPath();
                        this.context.arc(this.star[i][3], this.star[i][4], starSize, 0, Math.PI * 2);
                        this.context.fillStyle = `rgba(144, 238, 251, ${opacity})`;
                        this.context.fill();
                        this.context.closePath();
                        
                        // Optional: Add glow effect for larger/closer stars
                        if (starSize > 1.2) {
                            this.context.beginPath();
                            this.context.arc(this.star[i][3], this.star[i][4], starSize * 2.5, 0, Math.PI * 2);
                            var gradient = this.context.createRadialGradient(
                                this.star[i][3], this.star[i][4], 0,
                                this.star[i][3], this.star[i][4], starSize * 2.5
                            );
                            gradient.addColorStop(0, `rgba(144, 238, 251, ${opacity * 0.4})`);
                            gradient.addColorStop(1, 'rgba(144, 238, 251, 0)');
    ...
    // Initialize the starfield on the hero section with cyan/blue dots
    console.log('Initializing starfield with dots...');
    $('.hero-stars-canvas').starfield({
        starColor: "rgba(144, 238, 251, 1)", // Cyan color matching your theme
        bgColor: "rgba(0,0,0,0.08)",
        mouseMove: true,
        speed: 1.5,
        quantity: 512,
